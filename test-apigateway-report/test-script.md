...
Chứng minh **circuit breaker** trên Spring Cloud Gateway cho **order-service**: dùng **một Thread Group** gọi lần lượt ba endpoint smoke **`test` / `test1` / `test2`** để (1) xác nhận downstream sống, (2) tạo **slow call** đủ để breaker hướng tới **OPEN** (theo `slow_call_duration_threshold` / tỷ lệ slow call), (3) gọi endpoint **phản hồi nhanh** để sau khi chờ `wait_duration` / HALF_OPEN, breaker có thể về **CLOSED**. Khi breaker OPEN, gateway có thể trả **503** và body fallback (xử lý tại **`FallbackController.java`**, khoảng dòng 18).
